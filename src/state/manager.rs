...
impl StateManager {
    ...
    pub async fn handle_stream_online(
        ...
        match state.status {
            StreamStatus::Offline => {
                self.handle_offline_to_online(state, streamer_login, started_at)
                    .await;
            }
            ...
            StreamStatus::OfflinePending => {
                self.handle_offline_pending_to_online(state, streamer_login)
                    .await;
            }
        }
    }
        ...
        tokio::spawn(async move {
            ...
            if let Some(state) = states.get_mut(&streamer_id_clone) {
                ...
                if state.status == StreamStatus::OnlinePending {
                    state_manager
                        .confirm_online(state, &streamer_login_clone)
                        .await;
    ...
    async fn confirm_online(&self, state: &mut StreamState, streamer_login: &str) {
        info!(
